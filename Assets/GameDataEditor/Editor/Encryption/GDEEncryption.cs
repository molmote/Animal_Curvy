...
namespace GameDataEditor {
	public class GDEEncryption {
		...
		{
			if (!Directory.Exists(Path.GetDirectoryName(GDECodeGenConstants.CryptoFilePath)))
				Directory.CreateDirectory(Path.GetDirectoryName(GDECodeGenConstants.CryptoFilePath));
			...
			using (var stream = new MemoryStream())
			{
				...
				File.WriteAllText(GDECodeGenConstants.CryptoFilePath, Convert.ToBase64String(stream.ToArray()));
			}
