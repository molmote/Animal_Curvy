...
public class UIScrollView : MonoBehaviour
{
	...
	public float __slowingDownSpeed = 9f;
	...
	void LateUpdate ()
	{
		...
		if (!mPressed)
		{
			if (mMomentum.magnitude > 0.0001f || mScroll != 0f)
			{
				...
				// Move the scroll view
				Vector3 offset = NGUIMath.SpringDampen(ref mMomentum, GameUIManager.instance.scrollDampingMomentum, delta );
				//9f, delta);
				MoveAbsolute(offset);
		...
		else
		{
			// Dampen the momentum
			mScroll = 0f;
			NGUIMath.SpringDampen(ref mMomentum, GameUIManager.instance.scrollDampingMomentum, delta);
		}
	}
